...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    setUser(parsed);
    fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/vendors`).then(r => r.json()).then(setVendors).catch(() => {});
  ...
  async function toggleService(id: string) {
    await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/services/${id}/toggle`, { method: "PATCH", headers: { Authorization: `Bearer ${token}` } });
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/services/vendor/my`, { headers: { Authorization: `Bearer ${token}` } });
    setMyServices(Array.isArray(await res.json()) ? await res.json() : []);
  }

  async function deleteService(id: string) {
    if (!confirm(lang === "en" ? "Delete this service?" : "Service delete karein?")) return;
    await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/services/${id}`, { method: "DELETE", headers: { Authorization: `Bearer ${token}` } });
  ...
  const roleLabel = user.role === "vendor" ? "🏪 Vendor" : user.role === "pandit" ? "🕉️ Pandit" : user.role === "admin" ? "⚙️ Admin" : "👤 User";
  const quickLinks = tr.quickLinkItems[lang];
    ...
    <div style={{ minHeight: "80vh", background: "#f5f5f5", padding: "32px 20px" }}>
      <div style={{ maxWidth: 1000, margin: "0 auto" }}>
        ...
        <div style={{ background: "#b5451b", borderRadius: 16, padding: "28px 32px", color: "#fff", marginBottom: 24, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 16 }}>
          <div>
            <p style={{ fontSize: 14, opacity: 0.85, marginBottom: 4 }}>{t(tr.namaste)}</p>
            <h1 style={{ fontSize: 26, fontWeight: 900, marginBottom: 6 }}>{user.full_name || user.email}</h1>
            <span style={{ background: "rgba(255,255,255,0.2)", padding: "4px 14px", borderRadius: 20, fontSize: 13, fontWeight: 700 }}>{roleLabel}</span>
          </div>
        ...
        {(user.role === "vendor" || user.role === "pandit") && (
          <div style={{ background: "#fff", borderRadius: 16, padding: "24px", border: "1.5px solid #eee", marginBottom: 24 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20, flexWrap: "wrap", gap: 10 }}>
              <h2 style={{ fontSize: 20, fontWeight: 800, color: "#111" }}>{t(tr.myServices)} ({myServices.length})</h2>
            ...
            {myServices.length === 0 ? (
              <div style={{ textAlign: "center", padding: "32px 0", color: "#aaa" }}>
                ...
                <p style={{ marginBottom: 14 }}>{t(tr.noServiceYet)}</p>
                ...
                {myServices.map(s => (
                  <div key={s.id} style={{ border: `1.5px solid ${s.is_active ? "#eee" : "#fecaca"}`, borderRadius: 12, overflow: "hidden" }}>
                    <div style={{ height: 130, background: "#f5f5f5", display: "flex", alignItems: "center", justifyContent: "center", position: "relative", overflow: "hidden" }}>
                      {s.main_image ? <img src={s.main_image} alt={s.name} style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : <span style={{ fontSize: 40 }}>{ICONS[s.service_type] || "📦"}</span>}
        ...
        <div style={{ background: "#fff", borderRadius: 16, padding: "24px", border: "1.5px solid #eee" }}>
          <h2 style={{ fontSize: 20, fontWeight: 800, color: "#111", marginBottom: 18 }}>{t(tr.availableVendors)} ({vendors.length})</h2>
          {vendors.length === 0 ? (
            <div style={{ textAlign: "center", padding: "32px 0", color: "#aaa" }}>
              <div style={{ fontSize: 40, marginBottom: 10 }}>🔍</div>
              <p>{t(tr.noVendors)}</p>
