    ...
    <div style={{ minHeight: "80vh", background: "#f5f5f5", padding: "32px 20px" }}>
      <div style={{ maxWidth: 800, margin: "0 auto" }}>
        <div style={{ marginBottom: 24 }}>
          ...
          <h1 style={{ fontSize: 26, fontWeight: 900, color: "#111", marginTop: 4 }}>{t(tr.myOrders)}</h1>
          <p style={{ color: "#777", fontSize: 14 }}>{orders.length} {t(tr.totalOrders)}</p>
        </div>

        {loading ? (
          <div style={{ textAlign: "center", padding: 60, color: "#aaa" }}>{t(tr.loading)}</div>
        ) : orders.length === 0 ? (
          <div style={{ textAlign: "center", padding: 60, background: "#fff", borderRadius: 16, border: "1.5px solid #eee" }}>
            <div style={{ fontSize: 48, marginBottom: 12 }}>🛒</div>
            <p style={{ fontSize: 16, color: "#777", marginBottom: 20 }}>{t(tr.noOrders)}</p>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            {orders.map(o => {
              const s = STATUS_BG[o.status] || STATUS_BG.pending;
              const label = t(tr.statusLabels[o.status as keyof typeof tr.statusLabels] || tr.statusLabels.pending);
              const desc = t(tr.statusDesc[o.status as keyof typeof tr.statusDesc] || tr.statusDesc.pending);
              return (
                <div key={o.id} style={{ background: "#fff", borderRadius: 14, overflow: "hidden", border: `2px solid ${s.bg}` }}>
                  <div style={{ background: s.bg, padding: "10px 20px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                    <span style={{ color: s.color, fontWeight: 800, fontSize: 15 }}>{label}</span>
                    <span style={{ color: s.color, fontSize: 13 }}>{desc}</span>
                  </div>
                  <div style={{ padding: "18px 20px", display: "flex", gap: 16, flexWrap: "wrap" }}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <p style={{ fontSize: 14, color: "#555", marginBottom: 4 }}>
                        {ICONS[o.vendor_services?.service_type]} {o.vendor_services?.service_type} · <strong style={{ color: "#b5451b" }}>₹{o.vendor_services?.selling_price?.toLocaleString()}</strong>
                      </p>
                      <p style={{ fontSize: 14, color: "#555", marginBottom: 4 }}>
                        {t(tr.eventDate)} <strong>{new Date(o.event_date).toLocaleDateString(lang === "en" ? "en-IN" : "hi-IN")}</strong>
                      </p>
                      {o.message && <p style={{ fontSize: 13, color: "#777" }}>💬 {o.message}</p>}
                      <p style={{ fontSize: 12, color: "#aaa", marginTop: 6 }}>{t(tr.orderNum)}{o.id.slice(0, 8)}</p>
                    </div>
