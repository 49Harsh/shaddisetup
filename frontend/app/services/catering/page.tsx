...
export default function CateringPage() {
  ...
  const packages = tr.cateringPackages[lang];
    ...
    <div>
      <section style={{ background: "#fff", padding: "72px 20px", textAlign: "center", borderBottom: "2px solid #eee" }}>
        <div style={{ maxWidth: 700, margin: "0 auto" }}>
          ...
          <p style={{ fontSize: 18, color: "#555", lineHeight: 1.8, marginBottom: 32 }}>{t(tr.cateringDesc)}</p>
          <a href={`https://wa.me/919999999999?text=${lang === "en" ? "I need catering information" : "मुझे कैटरिंग के बारे में जानकारी चाहिए"}`}
      ...
      <section style={{ padding: "48px 20px", background: "#f5f5f5" }}>
        <div style={{ maxWidth: 900, margin: "0 auto", textAlign: "center" }}>
          <h2 style={{ fontSize: 28, fontWeight: 800, marginBottom: 32, color: "#111" }}>{t(tr.cateringWhat)}</h2>
          <div style={{ display: "flex", flexWrap: "wrap", gap: 16, justifyContent: "center" }}>
            {tr.cateringItems[lang].map((item, i) => (
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: 28 }}>
            {packages.map((p, i) => (
              <div key={i} style={{ border: i === 1 ? "2px solid #b5451b" : "1.5px solid #eee", borderRadius: 16, padding: "32px 24px", background: "#fff", position: "relative" }}>
                ...
                <a href={`https://wa.me/919999999999?text=${lang === "en" ? "I want to book catering" : "मुझे कैटरिंग बुक करना है"}`}
                  target="_blank" rel="noopener noreferrer"
